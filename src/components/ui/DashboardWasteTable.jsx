...
import { getReportsHistory } from "../../services/api"; // Adjust path as needed
...
const DashboardWasteTable = () => {
  const [sortConfig, setSortConfig] = useState({ key: "reportedDate", direction: "desc" });
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10; // Ensure this matches the backend limit if possible
  ...
  const [isApprovePickupModalOpen, setIsApprovePickupModalOpen] = useState(false);
  const [selectedReportId, setSelectedReportId] = useState(null);

  // Fetch waste history data
  useEffect(() => {
    const fetchWasteHistory = async () => {
      try {
        ...
        setWasteData(response.reports);
        setTotalItems(response.totalItems || 0);
        setError(null);
      } catch (err) {
        console.error("Error fetching waste history:", err);
        setError("Failed to load waste history. Please try again.");
        setWasteData([]);
        setTotalItems(0);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="overflow-x-auto wasteTable">
        <table className="min-w-full w-200 divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th
                scope="col"
                className="px-6 py-3 text-left text-[10px] sm:text-[11px] font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
                onClick={() => requestSort("reportedBy.name")}
              >
                <div className="flex items-center">
                  Reported By
                  {sortConfig.key === "reportedBy.name" && (
                    sortConfig.direction === "asc" ? (
                      <FiChevronUp className="ml-1" />
                    ) : (
                      <FiChevronDown className="ml-1" />
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              ...
              <tr>
                <td colSpan="5" className="px-6 py-4 text-center text-xs sm:text-[13px] text-red-500">
                  {error}
                </td>
              </tr>
            ) : wasteData.length > 0 ? (
                  ...
                  <td className="px-6 py-4 text-xs sm:text-[13px] text-gray-500 max-w-100">
                    <div>
                      ...
                      <p className="text-[11px] sm:text-xs mt-1">
                        Lat: {item.mapLocation?.latitude || "N/A"}, Long:{" "}
                        {item.mapLocation?.longitude || "N/A"}
                      </p>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-xs sm:text-[13px] text-gray-500">
...
export default DashboardWasteTable;